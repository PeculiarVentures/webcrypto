...
export class RsaOaepProvider extends core.RsaOaepProvider {
  ...
  public async onEncrypt(algorithm: types.RsaOaepParams, key: RsaPublicKey, data: ArrayBuffer): Promise<ArrayBuffer> {
    ...
    const labelHash = crypto.createHash(internalKey.algorithm.hash.name.replace("-", ""))
      .update(pvtsutils.BufferSourceConverter.toUint8Array(algorithm.label || new Uint8Array(0)))
      .digest();
  ...
  public async onDecrypt(algorithm: types.RsaOaepParams, key: RsaPrivateKey, data: ArrayBuffer): Promise<ArrayBuffer> {
   ...
   * @param length
   */
  protected mgf1(algorithm: types.Algorithm, seed: Uint8Array, length = 0) {
